    ...
    <div className={cn('w-full space-y-4', className)}>
      ...
      <div className="relative h-10 w-full flex items-center">
        ...
        <div
          ...
          style={{
            left: isRange
              ? `${((value as number[])[0] - min) / (max - min) * 100}%`
              : '0%',
            right: isRange
              ? `${100 - ((value as number[])[1] - min) / (max - min) * 100}%`
              : `${100 - ((value as number) - min) / (max - min) * 100}%`
          }}
        />
        ...
        {isRange ? (
          <>
            ...
            {/* Thumbs (Visual only) */}
            <div
              className="absolute top-1/2 h-6 w-6 rounded-full border-2 border-indigo-600 bg-white shadow-md pointer-events-none"
              style={{
